...
> This is just an updated versión of the original script to work with Python 3 and QGIS 3 (Oct 2018 by _LuisSevillano@iCarto_).
 I added some notes to the README too but all the work and recognition is for Alexander Hajnal.

[Demo](https://www.luissevillano.net/tile-writer/map/index.html).
# About
...
Running the script:
 ...
 - Define the map's area of interest:
   ...
   - Hide the layer because the canvas you see the output will be.
 - Adjust the settings to suit your needs (see below).
 - Note that all paths and filenames are relative to QGIS's current working directory or use absolute paths.
 ...
 - An _All done_ message will be printed when the script has finished.
...
# Rendering time
After several tests with an i7 2.60GHz machine).
...
# TODO
- Modify the "Splitting loop" to parallel processing in order to save time rendering. Any idea?
- Other improves to save time rendering.
